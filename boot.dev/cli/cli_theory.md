...
What Is a _Shell_? a programm that runs those commands from the terminal.
Shells do a lot of things, but their main job is to _interpret the commands you type and execute them._

# REPL

Shells are often referred to as "REPL"s. REPL stands for:
_Read_ - Read the commands you type
_Eval_ - Evaluate those commands, usually by running other programs on your computer
_Print_ - Print the output of those commands
_Loop_ - Give you a new prompt to type another command and repeat
...
- _directories_ are just containers
- _files_ are just a dump of _raw binary data_: 1's and 0's. The bytes in the file can actually represent anything:
  images, text, audio, video, etc.
- file system tree starts at the _root directory_ `/`

# FILE PATHS

The output of _pwd_ is a filepath.
...
Relative paths _do not start with_ `/` or `~`
...
_If you run a command with sudo that you don't understand, you could do serious damage to your system._
...
A _program_ is just a _set of instructions that a computer can execute_
An _Executable_ is a _file that contains this program_.
...
Programs that has been _converted from human-readable source code into binary machine code_.
_Machine code can be executed by the computer directly - each computer has a CPU that is designed for that._
Programming languages like GO, Rust, C++ are compiled languages.

# Interpreted

_Interpreted program is a program that needs another program to run - this program is usually called an interpreter._
...
_Shell scripts with .sh are also interpreted by the shell program._
...
_shebang_ is a special line at the top of the script that tells your shell which program to use as the interpreter.
...
Tip
You can also temporarily set a variable for a single command, instead of exporting it (_exporting means the variable will persist until you close the shell_).
...
_The PATH variable is a list of directories that your shell will look into when you try to run a command._
If you
...
To add a directory to your PATH _without overwriting_ all, of the existing directories, use the export command and
...
_-l_ long
_a_ - hidden files
_-al_ - combination of both

# Flags conventions

- single character flags are prefixed with a single dash _-a_
- multi character flags are prefixed with a double dash _--help_
...
- the way of communicating back by the programms
  _0 is the code for success. Any other code usually means an error._
...
The shell can _take an output of one program and use it as an input of another program_. This is possible thx to
